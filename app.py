...
# Custom formatter for Altman Z Score
...
option = st.sidebar.radio('Select a section:', 
                          ['Altman Z Score', 'Futures Pricing', 'JSON Conversion', 'Loan Pricing Calculator', 'Periodic Table of Commodity Returns'])
...
if option == 'Periodic Table of Commodity Returns':
    ...
    if st.button('Calculate Z-Scores'):
        for symbol in tickers:
            ...
            symbol_to_data[symbol] = {'Z-Score': zscore, 'X1': x1, 'X2': x2, 'X3': x3, 'X4': x4, 'X5': x5}
            if zscore <= 1.8:
